    ...
    for index, deck in enumerate(streamdeck): #Enumerate through all streamdecks (Only 1 right now)
        ...
        while 1: #While loop while thread is running
            ...
            if test2.is_set():
                ...
                set_briefcam()
                briefcam_login() 
                deck.set_key_image(7, PILHelper.to_native_key_format(deck, myicon.lpr_image))
                while not button_pressed_flag:
                    myicon.set_yellow_keys(deck)
                myicon.set_keys_normal(deck)
                lpr_yellow.clear()
                ...
                deck.set_key_image(7, PILHelper.to_native_key_format(deck, myicon.lpr_image))
                while not button_pressed_flag:
                    myicon.set_red_keys(deck)
                myicon.set_keys_normal(deck)
                lpr_red.clear()
            elif poi_yellow.is_set(): 
                set_briefcam()
                briefcam_login()                                                                                      
                deck.set_key_image(7, PILHelper.to_native_key_format(deck, myicon.poi_image))
                while not button_pressed_flag:
                    myicon.set_yellow_keys(deck)
                myicon.set_keys_normal(deck)
                poi_yellow.clear()
...
def on_about():
    ctypes.windll.user32.MessageBoxW(0, 'ATHEY DECK - v1.0.0-alpha', "Info", 0)
